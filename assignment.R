...
has_more_zs <- function(string1, string2){
  z1 <- str_count(string1, "z") + str_count(string1, "Z")
  z2 <- str_count(string2, "z") + str_count(string2, "Z")
  z3 <- z1 + z2
  if(z1 > z2) {
    return("String 1 contains more Zs.")
  } else if (z1 < z2) {
    return("String 2 contains more Zs.")
  } else if (z1 == z2) {
...
more_zs <- has_more_zs("zebra", "zzhhaz")
...
remove_digits(c("INFO 201", "MGMT 200", "INFO 360"))

### Vectors ###################################################################

# Create a vector `movies` that contains the names of six movies you like
movies <- c("Cars", "Titanic", "Borat", "Finding Nemo", "Madagascar", "Up")
...
summary_info <- list(numbers_len, numbers_mean, numbers_median)
...
summarize_vector <- function(vectnumbers) {
  result <- list(mean(vectnumbers), median(vectnumbers), length(vectnumbers))
  return(result)
}
...
summary_1_to_100 <- list(summarize_vector(1:100))
